...
class DepartmentCrawler(BaseCrawler):
    def __init__(self):
        ...
        self.progress_dir = Path(os.getenv('DEPARTMENT_PROGRESS_DIR', 'data/departments_progress'))
        self.data_dir = Path(os.getenv('DEPARTMENT_DATA_DIR', 'data/departments'))
        self.run_deadline_seconds = int(os.getenv('DEPARTMENT_RUN_DEADLINE_SECONDS', '17400'))
        self.flush_schools = max(1, int(os.getenv('DEPARTMENT_FLUSH_SCHOOLS', '25')))
    ...
    def get_progress_file(self):
        custom = os.getenv('DEPARTMENT_PROGRESS_FILE', '').strip()
        if custom:
            return Path(custom)
    ...
    def get_department_payload(self, school_id):
        url = f'https://static-data.gaokao.cn/www/2.0/school/department/{school_id}.json?a=www.gaokao.cn'
        try:
            ...
            print(f'      ⚠️  获取院系数据失败 (ID:{school_id}): {str(e)}')
        return None
    ...
    def normalize_school_payload(self, school, result):
        ...
        department_count = len(data) if isinstance(data, list) else 0
        special_count = 0
        if isinstance(data, list):
            for dept in data:
                if isinstance(dept, dict):
                    special_list = dept.get('special') or []
                    if isinstance(special_list, list):
                        special_count += len(special_list)
        return {
            'update_time': self.now_str(),
            'school_id': school_id,
            'school_name': school_name,
            'source_url': f'https://static-data.gaokao.cn/www/2.0/school/department/{school_id}.json?a=www.gaokao.cn',
    ...
    def crawl(self, schools=None):
        ...
        for school_index in range(start_index, len(schools)):
            ...
            result = self.get_department_payload(school_id)
            if not result or result == 'no_data' or not isinstance(result, dict):
                print(f' ✗ {school_name} - 无院系数据')
            ...
            if not self._first_logged:
                ...
                if isinstance(data, list) and data and isinstance(data[0], dict):
                    ...
                    special_list = data[0].get('special') or []
                    if isinstance(special_list, list) and special_list and isinstance(special_list[0], dict):
                        sample_special_keys = list(special_list[0].keys())
                print(f'\n\n   📡 [院系接口] school_id={school_id}')
                print(f'      URL: https://static-data.gaokao.cn/www/2.0/school/department/{school_id}.json?a=www.gaokao.cn')
                print('      ' + '─' * 50)
                print(f"      顶层键: {list(result.keys())}")
                print(f"      院系数量: {len(data) if isinstance(data, list) else 0}")
                print(f"      院系样例字段: {sample_keys}")
                print(f"      专业样例字段: {sample_special_keys}")
                print('      ' + '─' * 50 + '\n')
            ...
            print(f" ✓ {school_name} - 院系 {payload.get('department_count', 0)} 个，专业 {payload.get('special_count', 0)} 个")
